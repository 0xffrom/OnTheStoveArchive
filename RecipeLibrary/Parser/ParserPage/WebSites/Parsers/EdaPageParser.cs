        ...
        public RecipeShort[] Parse(IHtmlDocument document, IParserPageSettings settings)
        {
            ...
            var recipes =
                document.QuerySelectorAll(
                    "div.tile-list__horizontal-tile.horizontal-tile.js-portions-count-parent.js-bookmark__obj");
            ...
            foreach (var recipe in recipes)
            {
                ...
                if (divInfo != null)
                {
                    ...
                    string url = settings.Url + recipe.QuerySelector("div.horizontal-tile__item-link.js-click-link")
                        ?.Attributes[1]?.Value ?? "";
                    if (imageUrl != "" && title != "")
                        recipeShorts.Add(new RecipeShort(title, new ObjectsLibrary.Components.Image(imageUrl), url,
                            index));
                }
            }
